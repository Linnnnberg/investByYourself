        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card variant="interactive" onClick={() => window.location.href = '/analysis'}>
                <CardContent className="text-center p-6">
                  <div className="text-4xl mb-2">📈</div>
                  <h3 className="text-lg font-semibold mb-2">Market Analysis</h3>
                  <p className="text-gray-600">Deep dive into market trends</p>
                </CardContent>
              </Card>

              <Card variant="interactive" onClick={() => window.location.href = '/reports'}>
                <CardContent className="text-center p-6">
                  <div className="text-4xl mb-2">📊</div>
                  <h3 className="text-lg font-semibold mb-2">Generate Reports</h3>
                  <p className="text-gray-600">Create detailed investment reports</p>
                </CardContent>
              </Card>

              <Card variant="interactive" onClick={() => window.location.href = '/analysis/historical-data'}>
                <CardContent className="text-center p-6">
                  <div className="text-4xl mb-2">📅</div>
                  <h3 className="text-lg font-semibold mb-2">Historical Data</h3>
                  <p className="text-gray-600">View 5+ years of market data</p>
                </CardContent>
              </Card>

              <Card variant="interactive" onClick={() => window.location.href = '/operations'}>
                <CardContent className="text-center p-6">
                  <div className="text-4xl mb-2">⚙️</div>
                  <h3 className="text-lg font-semibold mb-2">Data Operations</h3>
                  <p className="text-gray-600">Manage data collection & processing</p>
                </CardContent>
              </Card>
            </div>
                        ...
                        <div key={portfolio.id} className="p-4 bg-gray-50 rounded-lg">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <div className={`text-xl font-bold ${(portfolio.change || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                {portfolio.total_gain_loss} ({portfolio.total_gain_loss_pct})
                              </div>
                            </div>
                          </div>
                          <div className="mt-2 text-sm text-gray-500">
                            Risk: {portfolio.risk_profile} • Holdings: {portfolio.holdings_count}
                          </div>
                        </div>
                      ))}
                    </div>
