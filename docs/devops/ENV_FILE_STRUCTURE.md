...
**Usage Option 1 - Automated Setup (RECOMMENDED)**:
```bash
cd services
python setup_env.py  # Auto-generates secure passwords
# Edit .env with your API keys
docker-compose up -d
```

**Usage Option 2 - Manual Setup**:
```bash
cd services
cp .env.example .env
# Edit .env with your values
...
**Why Automated Setup is Better**:
- ✅ Auto-generates secure random passwords (16-32 characters)
- ✅ No risk of weak passwords
- ✅ Faster setup (passwords in seconds vs minutes)
- ✅ Consistent configuration
- ⚠️ You still need to add API keys manually
...
| Use Case | File Location | Setup Method | Used By |
|----------|--------------|--------------|---------|
| Docker Compose Dev | `.env.example` | `cp .env.example .env` | `docker-compose.dev.yml` |
| **Microservices Dev** | `services/.env.example` | **`python setup_env.py`** ⭐ | `services/docker-compose.yml` |
| Microservices Prod | `services/.env.production.example` | `cp .env.production.example .env.production` | `services/docker-compose.prod.yml` |
| Standalone API | `api/.env.example` | `cp .env.example .env` | API service directly |
| Standalone Frontend | `frontend/.env.example` | `cp .env.example .env.local` | Next.js dev server |
| Standalone ETL | `etl/.env.example` | `cp .env.example .env` | ETL service directly |

⭐ **Recommended**: `setup_env.py` auto-generates secure passwords
...
### Existing: `services/setup_env.py`

**Purpose**: Automated environment setup for microservices with secure password generation

**Features**:
- ✅ Auto-generates 16-32 character secure passwords
...
cd services
python setup_env.py
# Follow prompts and add your API keys
```

**Generated Passwords Include**:
- POSTGRES_PASSWORD (20 chars)
- REDIS_PASSWORD (20 chars)
- MINIO_ROOT_PASSWORD (20 chars)
- JWT_SECRET_KEY (32 chars)
- PGADMIN_DEFAULT_PASSWORD (20 chars)
...
### Recommended: Create `setup_env.py` for Root

For consistency, create a root-level `setup_env.py`:

```python
#!/usr/bin/env python3
"""
Environment Setup Script for InvestByYourself Development
Creates a .env file for docker-compose.dev.yml
"""
...
def main():
    ...
    print("🚀 InvestByYourself Development Environment Setup")
    print("=" * 60)
    ...
    if env_file.exists():
        response = input("⚠️  .env exists. Overwrite? (y/N): ").strip().lower()
        if response != "y":
            print("Setup cancelled.")
            return

    env_content = f"""# InvestByYourself Development Environment
...
"""
    ...
    print("🚀 Start services: docker-compose -f docker-compose.dev.yml up -d")
