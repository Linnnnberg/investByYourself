...
def fetch_inflation_data():
    ...
    try:
        ...
        try:
            ...
            print(
                "⚠️  .env file has encoding issues. Using environment variable directly."
            )
...
def create_sample_data():
    ...
    sample_data = pd.DataFrame(
        {"CPIAUCSL": sample_cpi, "CPILFESL": sample_core_cpi, "PPIACO": sample_ppi},
        index=dates,
    )
...
def create_yoy_plot(data, yoy_data):
    ...
    ax.annotate(
        f"Latest CPI: {latest_cpi_yoy:.1f}%",
    ...
    ax.annotate(
        f"Latest Core CPI: {latest_core_yoy:.1f}%",
    ...
    ax.annotate(
        f"Latest PPI: {latest_ppi_yoy:.1f}%",
...
def main():
    ...
    print(
        f"Date Range: {inflation_data.index.min().strftime('%Y-%m-%d')} to {inflation_data.index.max().strftime('%Y-%m-%d')}"
    )
    ...
    print(
        f"Recent CPI trend: {'↗️ Increasing' if cpi_trend > 0 else '↘️ Decreasing'} ({cpi_trend:.2f}% per month)"
    )
    print(
        f"Recent Core CPI trend: {'↗️ Increasing' if core_trend > 0 else '↘️ Decreasing'} ({core_trend:.2f}% per month)"
    )
    print(
        f"Recent PPI trend: {'↗️ Increasing' if ppi_trend > 0 else '↘️ Decreasing'} ({ppi_trend:.2f}% per month)"
    )
