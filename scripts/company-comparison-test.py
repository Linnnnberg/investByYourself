...
def get_financial_ratios_for_comparison(
    symbols: List[str],
...
def generate_comparison_metrics(companies: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    comparison = {"companies": companies, "metrics": {}, "summary": {}}
    ...
    comparison["summary"] = {
        ...
        "sectors": list(
            set(company["sector"] for company in companies if company["sector"])
        ),
...
def generate_chart_data(
    companies: List[Dict[str, Any]], days: int = 365
) -> Dict[str, Any]:
    """Generate mock chart data for comparison visualization."""
    chart_data = {"companies": [], "data_points": []}
...
def main():
    ...
    print(
        f"✅ Comparison metrics generated for {comparison['summary']['total_companies']} companies"
    )
    ...
    print(
        "✅ Test results saved to data/test_output/company_comparison_test_results.json"
    )
