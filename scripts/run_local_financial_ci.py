...
class FinancialCIRunner:
    ...
    def __init__(self, skip_tests: bool = False, skip_security: bool = False, skip_integration: bool = False):
    ...
    def install_dependencies(self) -> bool:
        ...
        if not self.run_command(
            ...
            "Installing main financial dependencies"
        ):
            return False
    ...
    def run_financial_tests(self) -> bool:
        ...
        # Run with coverage
        if not self.run_command(
            [sys.executable, "-m", "pytest", "tests/", "--cov=scripts", "--cov-report=term-missing"],
    ...
    def run_code_quality_checks(self) -> bool:
        ...
        # Flake8 linting
        if not self.run_command(
            [sys.executable, "-m", "flake8", "scripts/", "tests/", "--max-line-length=88", "--extend-ignore=E203,W503"],
    ...
    def run_security_checks(self) -> bool:
        ...
        if not self.run_command(
            [sys.executable, "-m", "bandit", "-r", "scripts/", "-f", "json", "-o", "bandit-report.json"],
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Local Financial CI Runner for investByYourself")
    parser.add_argument("--skip-tests", action="store_true", help="Skip financial tests")
