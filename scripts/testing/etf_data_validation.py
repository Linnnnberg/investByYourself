# We'll generate a reusable Python script that:
# 1) Downloads 5Y of daily Adjusted Close data for XLK, XLF, XLE, XLU, and SPY from Yahoo Finance
# 2) Runs validation checks (missing dates, NaNs, non-positive prices, duplicate indices, extreme jumps)
# 3) Performs light cleaning (limit forward-fill for short gaps, drop bad rows if necessary)
# 4) Saves both raw and cleaned CSV files
# 5) Produces a concise validation report to STDOUT
#
# You can run this as:  python etf_data_validation.py --start 2019-01-01 --end 2025-08-25
#
# Note: This environment has no internet, but the script will work on your machine.
...
script_path = "/mnt/data/etf_data_validation.py"
...
def parse_args():
    ...
    p.add_argument("--start", type=str, default=(dt.date.today() - dt.timedelta(days=365*5)).isoformat(),
    ...
    p.add_argument("--outdir", type=str, default="./etf_data",
                   help="Output directory for CSVs and reports")
    p.add_argument("--max_ffill", type=int, default=3,
...
def nyse_business_days(start, end):
    ...
    idx = pd.bdate_range(start, end, freq='C')  # business days (Mon-Fri, no holidays)
...
def validate_prices(adj_close: pd.DataFrame, vol: pd.DataFrame, debug=False):
    ...
    report["global"]["index_monotonic_increasing"] = bool(not not_monotonic is False)
    ...
    for t in adj_close.columns:
        ...
        if start is not None and end is not None:
            biz = nyse_business_days(start, end)
        ...
        if debug:
            print(f"[DEBUG] {t}: obs={report['per_ticker'][t]['n_obs']} "
...
def print_report(report: dict):
    ...
    for t, d in report["per_ticker"].items():
        rows.append([t, d["n_obs"], d["nans"], d["zeros"], d["negs"],
                     d["extreme_return_days(>6σ)"], d["missing_business_days(approx)"]])
    if tabulate:
        print("\nPer-Ticker Summary:")
