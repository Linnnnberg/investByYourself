        ...
        comparison_data.append(
            {
                ...
                "Volume (M)": round(abs(company["volume"]) / 1e6, 1)
                if company["volume"]
                else 0,
            }
        )
...
def create_metrics_comparison_chart(companies: List[Dict[str, Any]]):
    ...
    plt.tight_layout()
    plt.savefig(
        "../data/test_output/company_metrics_comparison.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_performance_chart(companies: List[Dict[str, Any]]):
    ...
    for i, company in enumerate(companies):
        ...
        plt.plot(
            dates,
            prices,
            label=f"{company['symbol']} - {company['name']}",
    ...
    plt.tight_layout()
    plt.savefig(
        "../data/test_output/company_performance_comparison.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_sector_analysis(companies: List[Dict[str, Any]]):
    ...
    sector_market_caps = [
        sum(company["market_cap"] for company in sector_data[sector]) / 1e9
...
def main():
    ...
    comparison_df.to_csv(
        "../data/test_output/company_comparison_table.csv", index=False
    )
