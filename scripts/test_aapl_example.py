...
async def test_aapl_financial_transformation():
    ...
    print(f"   Company: {aapl_sample_data['company_name']} ({aapl_sample_data['symbol']})")
    ...
    if result.success:
        ...
        print(f"   Financial Statements: {len(transformed.get('financial_statements', {}))} statements")
        print(f"   Financial Metrics: {len(transformed.get('financial_metrics', {}))} metric categories")
        print(f"   Market Data: {len(transformed.get('market_data', {}))} market fields")
    ...
    for metric, value in valuation.items():
        if value is not None:
            if metric in ['pe_ratio', 'forward_pe', 'price_to_book', 'price_to_sales', 'ev_to_ebitda']:
    ...
    for metric, value in profitability.items():
        if value is not None:
            if metric in ['gross_margin', 'operating_margin', 'net_margin', 'roe', 'roa', 'roic']:
                print(f"   {metric.replace('_', ' ').title()}: {value:.2f}%")
    ...
    if not is_valid:
        print("   Note: Validation failed due to field name differences between source and standardized formats")
    ...
    batch_data = [
        ...
        {**aapl_sample_data, 'currentPrice': 180.0, 'marketCap': 3100000000000},  # Higher price scenario
        {**aapl_sample_data, 'currentPrice': 170.0, 'marketCap': 2900000000000}   # Lower price scenario
    ]
