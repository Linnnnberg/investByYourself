...
try:
    ...
    key_metrics = ['Gross Margin', 'Operating Margin', 'Net Profit Margin', 'Return on Equity', 'Return on Assets']
    
    # Create comparison dataframe
    comparison_data = []
    for metric in key_metrics:
        for year in years:
            if metric in aapl_ratios.index and year in aapl_ratios.index and pd.notna(aapl_ratios.loc[metric, year]):
            ...
            if metric in msft_ratios.index and year in msft_ratios.index and pd.notna(msft_ratios.loc[metric, year]):
                comparison_data.append({
                    'Year': year,
                    'Metric': metric,
                    'Company': 'MSFT',
    ...
    if not comparison_df.empty:
        ...
        fig.suptitle('Financial Performance Comparison: AAPL vs MSFT', fontsize=16, fontweight='bold')
        
        # Chart 1: Margin Comparison Over Time
        margin_data = comparison_df[comparison_df['Metric'].isin(['Gross Margin', 'Operating Margin', 'Net Profit Margin'])]
        if not margin_data.empty:
            sns.lineplot(data=margin_data, x='Year', y='Value', hue='Company', style='Metric', 
        ...
        if not data_2024.empty:
            pivot_data = data_2024.pivot(index='Metric', columns='Company', values='Value')
        ...
        return_data = comparison_df[comparison_df['Metric'].isin(['Return on Equity', 'Return on Assets', 'Return on Invested Capital'])]
        if not return_data.empty:
            sns.lineplot(data=return_data, x='Year', y='Value', hue='Company', style='Metric', 
        ...
        if len(available_metrics) > 0 and len(available_years) > 0:
            heatmap_matrix = np.zeros((len(available_metrics), len(available_years) * 2))
            
            for i, metric in enumerate(available_metrics):
                for j, year in enumerate(available_years):
                    # AAPL values
                    aapl_val = comparison_df[(comparison_df['Metric'] == metric) & 
                                           (comparison_df['Year'] == year) & 
                                           (comparison_df['Company'] == 'AAPL')]['Value'].values
                    if len(aapl_val) > 0:
                        heatmap_matrix[i, j*2] = aapl_val[0]
                    
                    # MSFT values
                    msft_val = comparison_df[(comparison_df['Metric'] == metric) & 
                                           (comparison_df['Year'] == year) & 
                                           (comparison_df['Company'] == 'MSFT')]['Value'].values
                    if len(msft_val) > 0:
            ...
            sns.heatmap(heatmap_matrix, annot=True, fmt='.3f', cmap='RdYlGn', 
                       xticklabels=years_labels, yticklabels=available_metrics, ax=axes[1,1])
        ...
        markdown_content.append("1. **Margin Trends Over Time**: Shows how Gross, Operating, and Net Profit margins evolved for both companies")
        markdown_content.append("2. **2024 Performance Comparison**: Direct comparison of key metrics for the most recent year")
        markdown_content.append("3. **Return Metrics Over Time**: Tracks Return on Equity, Assets, and Invested Capital trends")
        markdown_content.append("4. **Financial Metrics Heatmap**: Comprehensive view of all metrics across companies and years")
        markdown_content.append("")
...
markdown_content.append("The visual charts help identify trends and make direct comparisons between the two tech giants.")
markdown_content.append("")
markdown_content.append("---")
