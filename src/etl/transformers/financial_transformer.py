...
class FinancialRatioCalculator:
    ...
    @staticmethod
    def calculate_price_to_sales(price: float, sales_per_share: float) -> Optional[float]:
        ...
        if market_data:
            ...
            if price:
                ...
                if balance_sheet.get('book_value_per_share'):
                    metrics.price_to_book = self.ratio_calculator.calculate_price_to_book(
...
class FinancialDataTransformer(BaseDataTransformer):
    ...
    async def transform_data(
        ...
        try:
            ...
            standardized_data = {
                ...
                'financial_statements': self._extract_financial_statements(transformed_data),
    ...
    def _calculate_quality_metrics(
        ...
        present_fields = sum(1 for field in required_fields if field in transformed_data)
        ...
        if 'financial_statements' in transformed_data:
            income_stmt = transformed_data['financial_statements'].get('income_statement', {})
            if income_stmt.get('revenue') and income_stmt.get('net_income'):
                if income_stmt['revenue'] > 0 and income_stmt['net_income'] <= income_stmt['revenue']:
                    accuracy_score += 1.0
                accuracy_checks += 1
        ...
        if 'financial_metrics' in transformed_data:
            profitability = transformed_data['financial_metrics'].get('profitability', {})
